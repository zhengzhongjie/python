...
is to srart a program running indepandently here, not to connect to its streams;
multiprocessing module also is an option, but this is command-lines, not functions
doesn't make sense to start a pocess which would just do one of the options here;

new in this edition: runs script filename path through normpath() to change any
/ to \ for Windows tool where required; fix is inherited by PyEdit and others;
...
def fixWindowsPath(cmdline):
	"""
	channge all / to \ in script filename path at front of cmdline;
	used only bu classes which run tools thar require this on Windows;
...
class LaunchMode:
	...
	file to run, and not whit "python" or its full path;
	"""
...
class System(LaunchMode):
	...
	caveat: may block caller, unles & added on Unix
	"""
	def run(self, cmdline):
...
class Popen(LaunchMode):
	...
	cavaet: may block caller, since pipe closed too soon
	"""
	def run(self, cmdline):
		cmdline = fixWindowsPath(cmdline)
...
class Start(LaunchMode):
	"""
	run command indepandently of caller
